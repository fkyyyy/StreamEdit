...
# 967f: single-variable test — suppress source background VALUE injection.
# Base = 967e5 (raw posterior region, soft modulation, NO anchor write correction).
# Only change: source bg values replaced with target bg values after t^inj.
# Source bg keys remain unchanged so attention addressing is preserved.
...
OUTDIR="${OUTDIR:-$SCRIPT_DIR/outputs/967f_suppress_source_bg_value}"
OUTPUT_NAME="${OUTPUT_NAME:-967f-suppress-source-bg-value.mp4}"
...
COMMAND=(
  ...
  # THE SINGLE VARIABLE UNDER TEST: suppress source bg value injection
  --suppress_source_bg_value

  # NO anchor write correction (explicitly disabled)
  # --first_block_identity_anchor is NOT passed
...
{
  printf '%s\n' \
    'experiment=967f_suppress_source_bg_value' \
    'baseline=967e5_raw_posterior_region' \
    'single_variable=suppress_source_bg_value' \
    'anchor_write_correction=disabled' \
    'first_block_identity_anchor=disabled' \
    "data_path=$DATA_PATH"
  printf 'command='
  printf ' %q' "${COMMAND[@]}"
  printf '\n'
} > "$OUTDIR/967f_config.txt"

echo "967F single-variable test: suppress source bg value injection"
echo "967F baseline=967e5 change=src_bg_value→trg_bg_value"
echo "967F anchor_write_correction=disabled"
echo "967F_OUTPUT $OUTDIR/$OUTPUT_NAME"

if [[ "$DRY_RUN" == 1 ]]; then
  echo '967F_DRY_RUN resolved command:'
  printf ' %q' "${COMMAND[@]}"
  printf '\n'
  exit 0
fi
