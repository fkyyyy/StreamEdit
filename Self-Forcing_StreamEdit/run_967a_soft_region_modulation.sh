...
# 967a: soft region modulation.
# Uses the same flow-verified region detection as 966a, but the region acts as
# a continuous [0,1] spatial modulation signal on top of the native StreamEdit
# velocity blend, instead of hard-switching velocity/KV/attention routing.
# KV metadata falls back to the legacy cross-attention union mask.
...
OUTDIR="${OUTDIR:-$SCRIPT_DIR/outputs/967a_soft_region_modulation}"
OUTPUT_NAME="${OUTPUT_NAME:-967a-soft-region-modulation.mp4}"
...
if [[ -n "${SOURCE_OWNER_MASK:-}" || -n "${OBJECT_MASK:-}" ]]; then
  echo "967a forbids SOURCE_OWNER_MASK and OBJECT_MASK" >&2
  exit 2
fi
...
COMMAND=(
  ...
  # 967a: soft region modulation replaces hard routing
...
{
  printf '%s\n' \
    'experiment=967a_soft_region_modulation' \
  ...
  printf ' %q' "${COMMAND[@]}"
  printf '\n'
} > "$OUTDIR/967a_config.txt"

echo "967A_PROMPT src_word=$SRC_WORD trg_word=$TRG_WORD"
echo "967A_REGION proposal=token verified_by=clean_source_flow radius=$VERIFIED_OWNER_RADIUS"
echo "967A_VELOCITY native_soft_blend region_modulation=$SOFT_REGION_BLEND_STRENGTH"
echo '967A_KV legacy_cross_attention_union streamgve_dense_clean_target_history=enabled'
echo "967A_OUTPUT $OUTDIR/$OUTPUT_NAME"

if [[ "$DRY_RUN" == 1 ]]; then
  echo '967A_DRY_RUN resolved command:'
  printf ' %q' "${COMMAND[@]}"
  printf '\n'
  exit 0
fi
