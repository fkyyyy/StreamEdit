...
# Standalone 964 baseline: freeze the first three-frame target-KV proposal,
# then retrieve it through the automatic clean-source RGB-flow owner.
# This script calls Python directly and does not inherit another experiment.
...
OUTDIR="${OUTDIR:-$SCRIPT_DIR/outputs/964a_automatic_immutable_target_memory}"
OUTPUT_NAME="${OUTPUT_NAME:-964a-automatic-immutable-target-memory.mp4}"
PYTHON_BIN="${PYTHON_BIN:-python}"
CUDA_DEVICE="${CUDA_DEVICE:-7}"
DRY_RUN="${DRY_RUN:-0}"

STEP="${STEP:-15}"
IDENTITY_CORRECTION_STRENGTH="${IDENTITY_CORRECTION_STRENGTH:-0.60}"
...
if [[ -n "${SOURCE_OWNER_MASK:-}" || -n "${OBJECT_MASK:-}" ]]; then
  echo "964a forbids SOURCE_OWNER_MASK and OBJECT_MASK" >&2
  exit 2
fi
for argument in "$@"; do
  case "$argument" in
    ...
    --identity_support_floor|--identity_support_floor=*)
      echo "964a forbids mask, prompt, or identity-ablation override: $argument" >&2
      exit 2
      ;;
  esac
done
...
{
  printf '%s\n' \
    'experiment=964a_automatic_immutable_target_memory' \
    'entrypoint=direct_python_no_inherited_experiment_shells' \
    'identity_payload=first_three_frame_clean_target_kv_frozen' \
    ...
    'identity_value_mode=absolute_bounded' \
    "identity_correction_strength=$IDENTITY_CORRECTION_STRENGTH" \
    "identity_support_floor=$IDENTITY_SUPPORT_FLOOR" \
  ...
  printf ' %q' "${COMMAND[@]}"
  printf '\n'
} > "$OUTDIR/964a_config.txt"

echo "964A_PROMPT src_word=$SRC_WORD trg_word=$TRG_WORD"
echo "964A_TARGET $TRG_PROMPT"
echo "964A_IDENTITY mode=absolute strength=$IDENTITY_CORRECTION_STRENGTH layers=8,12,16,20 prototypes=$IMMUTABLE_TARGET_NUM_PROTOTYPES"
echo "964A_OUTPUT $OUTDIR/$OUTPUT_NAME"

if [[ "$DRY_RUN" == 1 ]]; then
  echo '964A_DRY_RUN resolved command:'
  printf ' %q' "${COMMAND[@]}"
  printf '\n'
  exit 0
fi
