...
class WanT2VCrossAttention(WanSelfAttention):
    ...
    def forward(self, x, context, context_lens, crossattn_cache=None):
        ...
        if crossattn_cache is not None and \
            ...
            #✨ prepare, the last half batch for trg
            ...
            if crossattn_cache.get("current_src_fg_mask", None) is not None:
                ...
                for b_idx in range(q_trg.shape[0]):
                    ...
                    if fg_mask[b_idx].any():
                        ...
                        b_x_trg_fg = b_x_trg_fg_num / b_x_trg_fg_denom
                        b_x_trg[:, fg_mask[b_idx]] = b_x_trg_fg
        ...
        '''✨ obtain cross attn mask
        crossattn_cache: 
            fg_indices: list[list[int]] (len=batch_size, e.g., [[2, 3], [3, 4], ..]) 
                        or single list[int] (e.g., [2, 3]).
            fg_scale: default 1, larger fg_scale will release the mask standard (larger mask).
            fg_mask_soft: soft mask in [-1, 1]
            # fg_mask_bin: binary mask in {0, 1}
        '''
        if crossattn_cache is not None and \
                crossattn_cache.get("fg_indices", None) is not None and \
                crossattn_cache.get("obtain_mask", False):
            # len of crossattn_cache["fg_indices"] should be 1 or batch_size
            if not isinstance(crossattn_cache["fg_indices"][0], list):
                fg_indices = [crossattn_cache["fg_indices"]]
            else:
                fg_indices = crossattn_cache["fg_indices"]
            # foreground: 1 * fg_scale / n_fg_token, background: -1 / (Lk - n_fg_token)
            mask_value_list = []
            for fg_idx in fg_indices:
                # exp of one fg_idx item: [2, 3] (list of int index)
                m_value = torch.zeros_like(v[: 1])     # [1, Lk, Nk, dim]
                # all background for null trigger
                if len(fg_idx):
                    m_value += -1 / (v.size(1) - len(fg_idx))
                    m_value[:, fg_idx] = crossattn_cache.get("fg_scale", 1) / len(fg_idx)
                mask_value_list.append(m_value)
            if len(mask_value_list) == 1:
                mask_value_list = mask_value_list * b
            # mean heads
            mask_value = torch.cat(mask_value_list, dim=0)
            mask_soft: torch.Tensor = flash_attention(q, k, mask_value, k_lens=context_lens).mean(dim=[2, 3], keepdim=True)
            # drop getting mask condition
