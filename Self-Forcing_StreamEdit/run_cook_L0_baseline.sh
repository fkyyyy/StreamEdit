...
# Cook L0: baseline — native StreamGVE, all experimental flags OFF.
...
OUTDIR="${OUTDIR:-$SCRIPT_DIR/outputs/cook_L0_baseline}"
OUTPUT_NAME="${OUTPUT_NAME:-cook-L0-baseline.mp4}"
...

{
  ...
  printf ' %q' "${COMMAND[@]}"
  printf '\n'
} > "$OUTDIR/cook_L0_config.txt"

echo "Cook L0 baseline: metal spatula → wooden spatula, all flags OFF"
