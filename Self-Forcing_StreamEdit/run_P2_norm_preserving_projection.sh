...
# P2: P0 projected source residual + norm-preserving calibration.
#
# Single change from P0: after removing the antagonistic source residual,
# restore its application-weighted norm using one positive scalar per sample
# and latent timestep. This changes magnitude only; the P0-safe direction is
# retained.
#
...
OUTDIR="${OUTDIR:-$SCRIPT_DIR/outputs/P2_norm_preserving_projection}"
OUTPUT_NAME="${OUTPUT_NAME:-P2-norm-preserving-projection.mp4}"
...
COMMAND=(
  ...
  # P0 projection plus the single P2 change:
...
{
  printf '%s\n' \
    'experiment=P2_norm_preserving_projection' \
    'baseline=P0_projected_residual' \
    'single_variable=application_weighted_projected_residual_norm' \
    'norm_scope=global_per_sample_per_latent_timestep_no_spatial_region' \
    'norm_scale_guard=4.0' \
  ...
  printf ' %q' "${COMMAND[@]}"
  printf '\n'
} > "$OUTDIR/P2_config.txt"

echo "P2: P0 + application-weighted norm-preserving projection"
echo "P2 keeps native StreamGVE attention/KV/SOG and adds no spatial region"
echo "P2_OUTPUT $OUTDIR/$OUTPUT_NAME"

if [[ "$DRY_RUN" == 1 ]]; then
  echo 'P2_DRY_RUN resolved command:'
  printf ' %q' "${COMMAND[@]}"
  printf '\n'
  exit 0
fi
