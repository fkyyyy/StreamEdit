    ...
    def forward(
        ...
        if kv_cache is None:
            ...
            if sink_tokens > 0:
                ...
                attn_key = k_cat
                attn_value = v_cat
                attn_seq_slice_list = [slice(0, sink_tokens), slice(local_start_for_window, local_end_index)]
            else:
                window_start = max(0, local_end_index - self.max_attention_size)
                attn_key = temp_k[:, window_start:local_end_index]
                attn_value = temp_v[:, window_start:local_end_index]
                attn_seq_slice_list = [slice(window_start, local_end_index)]

            # ====================================================================================================
            # generation or editing
            # ====================================================================================================
            cache_update_info["sink_tokens"] = sink_tokens
            cache_update_info["num_new_tokens"] = num_new_tokens
            ...
            if kv_cache.get("trg_fg_mask", None) is None or kv_cache.get("current_src_fg_mask", None) is None:
                ...
                blender_rate = 1 - kv_cache['shared_dict']['current_timestep'] ** 2
                ...
                for b_idx in range(b // 2):
                    ...
                    b_trg_fg_mask = kv_cache["trg_fg_mask"][b_idx]
                    b_trg_attn_fg_mask = torch.cat([
                        b_trg_fg_mask[ass] for ass in attn_seq_slice_list
                    ], dim=0)
